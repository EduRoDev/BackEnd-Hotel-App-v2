...
func UserRoutes(router *mux.Router, logger *log.Logger) {
	...
	router.HandleFunc("/user/login", wrapHandler(userController.Login)).Methods("POST")
...
func PaymentRoutes(router *mux.Router, logger *log.Logger) {
	...
	router.HandleFunc("/pago/reserva/{id:[0-9]+}", wrapHandler(paymentController.Cancel)).Methods("PUT")
}

func AdminRoutes(router *mux.Router, logger *log.Logger) {
	adminController := controllers.NewAdminController(logger)
	router.HandleFunc("/admin/login", wrapHandler(adminController.Login)).Methods("POST")
	router.HandleFunc("/admin", wrapHandler(adminController.Create)).Methods("POST")
	router.HandleFunc("/admin", wrapHandler(adminController.GetAdministradoresHandler)).Methods("GET")
}
